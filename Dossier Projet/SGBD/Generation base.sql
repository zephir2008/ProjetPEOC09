...
CREATE TABLE IF NOT EXISTS `g_et_co_bdtheque`.`gco_livres` (
  ...
  `liv_auteur` VARCHAR(45) NOT NULL,
  `liv_photo` VARCHAR(255) NULL,
  `liv_en_stock` INT NOT NULL DEFAULT 0,
  `liv_appro` INT NOT NULL DEFAULT 0,
  `gco_fournisseur_id_fournisseur` INT NOT NULL,
  `gco_etat_livre_id_etat` INT NOT NULL,
...
USE `g_et_co_bdtheque`;
DROP procedure IF EXISTS `g_et_co_bdtheque`.`autocomplete`;
SHOW WARNINGS;

DELIMITER $$
USE `g_et_co_bdtheque`$$
create procedure autocomplete(in lke varchar(25))
begin
	set @cmpr = '%'+lke+'%';
SELECT DISTINCT
    'A' + CAST(id_livre AS CHAR) AS id,
    liv_title + ' (' + liv_annee_edition + ')' AS 'value'
FROM
    gco_livres
WHERE
    val LIKE @cmpr
...
UNION SELECT DISTINCT
    'A' + CAST(id_livre AS CHAR) AS id,
    'Editeur: ' + liv_editeur AS 'value'
FROM
    gco_livres
WHERE
    val LIKE @cmpr
ORDER BY liv_editeur 
UNION SELECT DISTINCT
    'F' + CAST(id_fournisseur AS CHAR) AS id,
    'Fournisseur: ' + frn_nom AS 'value'
FROM
    gco_fournisseurs
WHERE
    val LIKE @cmpr
...
CREATE  OR REPLACE VIEW `view_bibliotheque` AS
    ...
    FROM
        ...
        gco_fournisseurs ON gco_fournisseur_id_fournisseur = id_fournisseur
            INNER JOIN
        gco_etat_livre ON gco_etat_livre_id = id_etat
